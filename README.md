...
В каждую запись попадают: имя пресета, режим запуска, код выхода, время работы процесса, CPU/память процесса, агрегированные игровые метрики (`fps`, `frame_time_ms`, `cpu_percent`, `gpu_percent`) и ключевые параметры выбранного конфига.
