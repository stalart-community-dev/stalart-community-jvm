...
- `service.exe` — тихий перехватчик. Регистрируется в IFEO как `Debugger` для `stalcraft.exe` / `stalcraftw.exe` и запускается Windows автоматически при старте игры. Не имеет UI.
...
Когда запускается `stalcraft.exe` или `stalcraftw.exe`, Windows вместо самой игры стартует `service.exe`, передавая ему исходные аргументы лаунчера. Дальнейшее поведение `service.exe`:
...
Отдельного JVM/GC лога нет — STALCRAFT использует кастомную сборку OpenJDK 9 без поддержки `-Xlog` и `-Xloggc` в CLI, поэтому направить unified logging в файл невозможно. Для большинства проблем достаточно `wrapper.log`.
